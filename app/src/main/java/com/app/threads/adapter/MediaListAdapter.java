    ...
    public void onBindViewHolder(MyViewHolder holder, final int position) {
        ...
        if (item.getImageUrl().length() > 0) {
            ...
            if (item.getSelectedImageFileName().length() > 0) {
                ...
                holder.thumbnail.setImageURI(FileProvider.getUriForFile(ctx, BuildConfig.APPLICATION_ID + ".provider", new File(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_PICTURES) + File.separator + item.getSelectedImageFileName())));
