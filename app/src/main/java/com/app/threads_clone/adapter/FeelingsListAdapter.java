...
public class FeelingsListAdapter extends RecyclerView.Adapter<FeelingsListAdapter.MyViewHolder> {
	...
	private final List<Feeling> itemList;
	...
	public static class MyViewHolder extends RecyclerView.ViewHolder {
		...
		public MyViewHolder(View view) {
			...
			tvfeelingName = (TextView) view.findViewById(R.id.tvfeelingName);
	...
	public void onBindViewHolder(final MyViewHolder holder, final int position) {
		...
		holder.tvfeelingName.setVisibility(View.GONE);
		...
		if (item.getImgUrl() != null && item.getImgUrl().length() > 0) {
			...
			final TextView text = holder.tvfeelingName;
			final ProgressBar progressView = holder.mProgressBar;

			 holder.tvfeelingName.setText(item.getTitle());
